...
Stateful agent with tool execution and event streaming. Built on `@SatoPi/pi-ai`.

## Installation

```bash
npm install @SatoPi/pi-agent
```

## Quick Start

```typescript
import { Agent } from "@SatoPi/pi-agent";
...
```typescript
declare module "@SatoPi/pi-agent" {
...
Define tools using `AgentTool` with a Zod parameter schema (via `z` from `@SatoPi/pi-ai`).

```typescript
import { z } from "@SatoPi/pi-ai";
...
```typescript
import { Agent, streamProxy } from "@SatoPi/pi-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@SatoPi/pi-agent";
