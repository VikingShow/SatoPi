/**
 * Centralized path helpers for stp config directories.
 *
 * Uses PI_CONFIG_DIR (default ".stp") for the config root and
 * PI_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/stp/. This requires running `stp config init-xdg` first to
 * move data to the new locations. No filesystem existence checks are performed
 * — if the env var is set, stp trusts that the migration has been done.
 */
...
/**
 * Resolve the active profile from the two profile env vars. `STP_PROFILE` is the
 * canonical variable and takes precedence; `PI_PROFILE` is the legacy
 * compatibility fallback, consulted only when `STP_PROFILE` is undefined. An
 * explicitly-empty `STP_PROFILE` therefore selects the default profile rather
...
function getProfileFromEnv(): string | undefined {
	return resolveProfileEnv(process.env.STP_PROFILE, process.env.PI_PROFILE);
}

/**
 * Module-load profile resolution. Unlike {@link getProfileFromEnv}, an invalid
 * STP_PROFILE/PI_PROFILE value does NOT throw here — a bad env var must not
 ...
 * user still gets a clean "Invalid STP profile" message.
 */
...
/** Get the config directory name relative to home (e.g. ".stp" or PI_CONFIG_DIR override). */
export function getConfigDirName(): string {
	return process.env.PI_CONFIG_DIR || CONFIG_DIR_NAME;
}

/** Get the config agent directory name relative to home (e.g. ".stp/agent" or PI_CONFIG_DIR + "/agent"). */
...
/**
 * Resolves and caches all stp directory paths. On Linux, when XDG environment
 * variables are set, paths are redirected under $XDG_*_HOME/stp/. A new
...
class DirResolver {
	...
	// With XDG on Linux, they point to $XDG_*_HOME/stp/.
	...
	constructor(options: { agentDirOverride?: string; profile?: string } = {}) {
		...
		// resolves under $XDG_*_HOME/stp once `stp config init-xdg` has migrated
		...
		// profile could resolve to `~/.stp/profiles/<name>` on first activation
		// (when no $XDG_*_HOME/stp exists yet) and then silently move to
		// `$XDG_*_HOME/stp/profiles/<name>` the moment the base appeared, orphaning
		...
		// XDG flattens the agent/ prefix: ~/.stp/agent/sessions → $XDG_DATA_HOME/stp/sessions
		this.#agentDirs = {
 ...
 * `PI_PROFILE` that was bypassed because `STP_PROFILE` explicitly selected the
 ...
 * `~/.stp/agent`. Captured at module load — ignoring a profile-derived value
...
/** Get the config root directory (~/.stp). */
...
export function setProfile(profile: string | undefined): void {
	...
	if (activeProfile) {
		...
		process.env.STP_PROFILE = activeProfile;
...
/** Get the agent config directory (~/.stp/agent). */
export function getAgentDir(): string {
	return dirs.agentDir;
}

/** Get the project-local config directory (.stp). */
export function getProjectAgentDir(cwd: string = getProjectDir()): string {
	return path.join(cwd, CONFIG_DIR_NAME);
}

// =============================================================================
// Config-root subdirectories (~/.stp/*)
// =============================================================================

/** Get the reports directory (~/.stp/reports). */
export function getReportsDir(): string {
	return dirs.rootSubdir("reports", "state");
}

/** Get the logs directory (~/.stp/logs). */
export function getLogsDir(): string {
	return dirs.rootSubdir("logs", "state");
}

/** Get the path to a dated log file (~/.stp/logs/stp.YYYY-MM-DD.log). */
export function getLogPath(date = new Date()): string {
	return path.join(getLogsDir(), `${APP_NAME}.${date.toISOString().slice(0, 10)}.log`);
}

/**
 * Get the plugins directory (~/.stp/plugins or its XDG equivalent).
 *
 ...
 * (PR checkout, task isolation) and cleanup (`stp worktree`). A relative value
 * would resolve against whatever cwd happened to launch `stp`, so checkout and
...
/** Get the remote host info directory (~/.stp/remote-host). */
export function getRemoteHostDir(): string {
	return dirs.rootSubdir("remote-host", "data");
}

/** Get the managed Python venv directory (~/.stp/python-env). */
...
/** Get the puppeteer sandbox directory (~/.stp/puppeteer). */
 ...
 * (e.g. PR-checkout and task-isolation worktree dirs under `~/.stp/wt/`).
...
/** Get the path to a single worktree directory (~/.stp/wt/<segment>). */
export function getWorktreeDir(segment: string): string {
	return path.join(getWorktreesDir(), segment);
}

/** Get the GPU cache path (~/.stp/gpu_cache.json). */
...
/**
 * Get the encrypted auth-broker snapshot cache path (~/.stp/cache/auth-broker-snapshot.enc).
 * Honors the `STP_AUTH_BROKER_SNAPSHOT_CACHE` env var when set so tests and
 * operators can isolate or relocate the cache file.
 */
export function getAuthBrokerSnapshotCachePath(): string {
	const override = process.env.STP_AUTH_BROKER_SNAPSHOT_CACHE;
	if (override) return override;
	return dirs.rootSubdir(path.join("cache", "auth-broker-snapshot.enc"), "cache");
}

/** Get the local FastEmbed model cache directory (~/.stp/cache/fastembed). */
export function getFastembedCacheDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed"), "cache");
}

/** Get the on-demand fastembed runtime install root (~/.stp/cache/fastembed-runtime). */
export function getFastembedRuntimeDir(): string {
	return dirs.rootSubdir(path.join("cache", "fastembed-runtime"), "cache");
}

/** Get the natives directory (~/.stp/natives). */
export function getNativesDir(): string {
	return dirs.rootSubdir("natives", "cache");
}

/** Get the stats database path (~/.stp/stats.db). */
export function getStatsDbPath(): string {
	return dirs.rootSubdir("stats.db", "data");
}

/** Get the autoresearch state directory (~/.stp/autoresearch). */
export function getAutoresearchDir(): string {
	return dirs.rootSubdir("autoresearch", "state");
}

/** Get the per-project autoresearch state directory (~/.stp/autoresearch/<encoded-project>). */
export function getAutoresearchProjectDir(encodedProject: string): string {
	return path.join(getAutoresearchDir(), encodedProject);
}

/** Get the per-project autoresearch SQLite database path (~/.stp/autoresearch/<encoded-project>.db). */
export function getAutoresearchDbPath(encodedProject: string): string {
	return path.join(getAutoresearchDir(), `${encodedProject}.db`);
}

/** Get the per-run artifact directory (~/.stp/autoresearch/<encoded-project>/runs/<runId>). */
...
// =============================================================================
// Agent subdirectories (~/.stp/agent/*)
// =============================================================================
...
/** Get the last-seen-changelog-version marker file (~/.stp/agent/last-changelog-version). */
...
/** Get the tiny title model cache directory (~/.stp/agent/cache/tiny-models). */
...
/**
 * Persistent per-install UUID stored at `~/.stp/install-id`.
 *
 ...
 * `~/.stp/agent/` does not regenerate it. Server-side dedup for grievance
 * pushes (and similar telemetry) keys on this id.
 *
 * Anchored to the base config root (`~/.stp/install-id`) regardless of the
