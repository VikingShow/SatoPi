...
import { formatHashlineHeader, InMemorySnapshotStore } from "@satopi/hashline";
import type { AgentMessage, ResolvedThinkingLevel, ThinkingLevel } from "@satopi/pi-agent-core";
import type { Model, ToolExample } from "@satopi/pi-ai";
import { formatSessionDumpText, RpcClient } from "@satopi/pi-coding-agent";
import { prompt } from "@satopi/pi-utils";
import { diffLines } from "diff";
import { formatDirectory } from "@satopi/typescript-edit-benchmark/formatter";
import {
	discoverSharedInfra,
	InProcessClient,
	type SharedInfra,
} from "@satopi/typescript-edit-benchmark/in-process-client";
...
import type { EditTask } from "@satopi/typescript-edit-benchmark/tasks";
import {
	verifyExpectedFileSubset,
	verifyExpectedFiles,
} from "@satopi/typescript-edit-benchmark/verify";

const REPO_ROOT = path.resolve(import.meta.dir, "..", "..", "..", "..");
const RUNS_DIR = path.join(REPO_ROOT, "runs");
const TMP = path.join(RUNS_DIR, `rb-${Math.random().toString(36).slice(2, 10)}`);
const CLI_PATH = Bun.fileURLToPath(import.meta.resolve("@satopi/pi-coding-agent/cli"));
