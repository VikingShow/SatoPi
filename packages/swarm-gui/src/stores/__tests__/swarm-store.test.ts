...
describe("SwarmStore: before-loop interactions", () => {
  it("has expected actions on store", () => {
    const store = getStore();
    expect(typeof store.startPlanning).toBe("function");
    expect(typeof store.cancelBeforeLoop).toBe("function");
    expect(typeof store.sendSteering).toBe("function");
    expect(typeof store.pauseRun).toBe("function");
    expect(typeof store.resumeRun).toBe("function");
    expect(typeof store.resolveBlocker).toBe("function");
  });
