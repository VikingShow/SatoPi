...
import { ApiError } from "@oh-my-pi/pi-web/fetch";
...
// Module-level refs for cleanup on re-init.  Because the __initRunning guard
// is explicitly cleared on session switch / 404-recovery, each subsequent
// init() would otherwise start another 5 s polling interval and register
// another set of SSE listeners — causing duplicate events and cascading API
// calls.  Holding these refs lets us unwind the previous registration first.
let _pollInterval: ReturnType<typeof setInterval> | null = null;
let _unsubSse: (() => void) | null = null;
let _unsubConn: (() => void) | null = null;
...
interface SwarmStore {
  ...
  error: string | null;
  /** Dedicated streaming state — updated token-by-token outside the messages
   *  Map so that stream deltas don't trigger a full Zustand→React→virtualizer
   *  cycle.  Only ChatView subscribes; the virtualizer stays idle. */
  streamingMessage: {
    id: string;
    from: string;
    body: string;
...
export const useSwarmStore = create<SwarmStore>((set, get) => ({
  ...
  init: async () => {
    ...
    // Tear down prior listeners & interval before re-registering.
    // Without this every re-init (session switch, 404 recovery) leaks
    // one SSE listener + one onConnectionChange listener + one 5 s poll
    // interval, causing duplicate events and cascading API calls.
    _unsubConn?.();
    _unsubSse?.();
    ...
    try {
      ...
      _unsubConn = sseClient.onConnectionChange(onConnChange);
      ...
      try {
        ...
        if (apiErr instanceof ApiError && apiErr.category === "notFound") {
        ...
        set({ error: apiErr instanceof ApiError ? apiErr.message : String(apiErr) });
      }
      ...
      const flushDelta = () => {
        ...
        if (deltaBuffer) {
          ...
          deltaBuffer = null;
          // Update streamingMessage directly — bypasses the messages Map
          // and virtualizer.  Each token batch only triggers a lightweight
          // re-render of the StreamingBubble, not the full message list.
          set((s) => ({
            streamingMessage: s.streamingMessage
              ? { ...s.streamingMessage, body: s.streamingMessage.body + buffered.body, ts: buffered.ts }
              : null,
          }));
        }
      };
      ...
      _unsubSse = sseClient.on((entry) => {
        // Only touch connection status when it's NOT already live — avoids a
        // redundant Zustand merge + subscriber notification on every single
        // SSE event (including high-frequency stream_delta tokens).
        if (get().connectionStatus !== "live") {
          set({ isConnected: sseClient.isConnected, connectionStatus: "live" });
        }

        // stream_start: create a streaming bubble outside the messages Map
        // so subsequent token deltas don't trigger virtualizer recalculation.
        ...
        // Fast path: buffer stream_delta, update streamingMessage on flush.
        ...
        // stream_end: finalize the streaming bubble into the messages Map.
        // The accumulated body lives in streamingMessage (updated by flushDelta
        // above).  We add the completed message to the Map and clear
        // streamingMessage so the virtualizer picks it up from then on.
        if (entry.type === "stream_end" && entry.from) {
          const sm = get().streamingMessage;
          if (sm) {
            const finalBody = sm.body || entry.body || "";
            set((state) => {
              ...
              msgList.push({
                ...
                to: "all",
                body: finalBody,
                thinking: entry.thinking || sm.thinking,
                streaming: false as const,
                timestamp: entry.ts,
              } as ChatMessage);
              messages.set("roundtable", msgList);
              return { activities, messages, streamingMessage: null };
            });
          } else {
            // History replay or legacy: streamingMessage wasn't set — fall
            // back to addActivity which handles the legacy stream_start/delta
            // → stream_end flow inside the messages Map.
      ...
      _pollInterval = setInterval(() => get().refreshState(), 5000);
    } catch (err) {
    ...
    set((state) => {
      ...
      if (entry.type === "stream_start" && entry.from) {
        ...
        msgList.push({
          ...
          // Track stream creation time for TTFT measurement and gap detection.
          streamCreatedAt: entry.ts,
          timestamp: entry.ts,
        } as ChatMessage);
        messages.set("roundtable", msgList);
      ...
      if (entry.type === "stream_delta" && entry.from) {
        ...
        if (lastMsg && lastMsg.id.startsWith(`stream-`)) {
          ...
          // Update timestamp on each delta so the UI can detect streaming gaps
          // (gap = Date.now() - msg.timestamp > threshold).
      ...
      if (entry.type === "stream_end" && entry.from) {
        ...
        //
        // However, when stream_end carries a COMPLETELY DIFFERENT body (e.g.
        // deltas captured the model's questions while stream_end delivers the
        // final plan.md), appending with a separator is less jarring than
        // silently replacing the text the user has been reading.
        const msgList = [...(messages.get("roundtable") ?? [])];
        const lastMsg = msgList[msgList.length - 1];
        if (lastMsg && lastMsg.id.startsWith("stream-")) {
          const deltaBody = lastMsg.body;
          const endBody = entry.body || "";
          let finalBody: string;
          if (deltaBody && endBody && deltaBody.length <= endBody.length) {
            // stream_end body is longer — but is it the same content extended,
            // or completely different?
            const deltaNorm = deltaBody.replace(/\s+/g, " ").trim();
            const endNorm = endBody.replace(/\s+/g, " ").trim();
            // If the delta body is NOT a prefix of the end body (i.e. it was
            // a "thinking out loud" preamble), keep both with a divider so the
            // user sees the conversation flow rather than a sudden text swap.
            if (!endNorm.includes(deltaNorm.slice(0, Math.max(20, deltaNorm.length / 2)))) {
              finalBody = deltaBody + "\n\n---\n\n" + endBody;
            } else {
              finalBody = endBody;
            }
          } else {
            finalBody = deltaBody && deltaBody.length > endBody.length
              ? deltaBody
              : (endBody || deltaBody);
          }
          msgList[msgList.length - 1] = {
