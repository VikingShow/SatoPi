...
function MessageBubble({ msg }: { msg: ChatMessage }) {
  ...
  // ── Gap detection for streaming messages ──────────────────────────
  // Re-render every second while streaming so we can show a "Thinking…"
  // indicator when the model pauses between token bursts (7-55 s gaps).
  ...
  const gapSecs = msg.streaming && msg.body
    ? Math.floor((Date.now() - msg.timestamp) / 1000)
    : 0;
  const showThinking = gapSecs > 5;
  const ttftSecs = msg.streaming && !msg.body && msg.streamCreatedAt
    ? Math.floor((Date.now() - msg.streamCreatedAt) / 1000)
    : 0;
        ...
        {msg.streaming && !msg.body ? (
          <div className="flex flex-col items-start gap-0.5 py-0.5">
            <span className="text-xs text-muted-foreground/60 italic">
              {ttftSecs < 3 ? "Generating response…" : ttftSecs < 8 ? "Still working…" : "Taking longer than expected…"}
            </span>
          ...
          <>
            ...
            {/* Streaming gap heartbeat: "Thinking…" when no tokens for > 5 s */}
            {showThinking && (
              <div className="flex items-center gap-1.5 mt-1.5 text-[11px] text-muted-foreground/50 italic select-none">
                <span className="inline-block w-1.5 h-1.5 rounded-full bg-primary/50 animate-pulse" />
                Thinking{gapSecs > 15 ? `… (${gapSecs}s)` : "…"}
              </div>
            )}
          </>
        )}
...
export default function ChatView() {
  ...
  const streamingMessage = useSwarmStore((s) => s.streamingMessage);
  ...
  // Memoised so the downstream displayMessages useMemo (which feeds the
  // virtualizer) sees a stable reference and only recomputes when the
  // activity log actually changes — not on every render.
  ...
  // (used for virtualizer scroll tracking when messages are inside it).
  // For streaming messages rendered outside the virtualizer, see the
  // streamingMessage effect below.
  ...
  // Auto-scroll while streaming — the streaming bubble lives outside the
  // virtualizer, so we track its body length directly to keep the latest
  // token visible without recalculating the entire virtual list.
  const streamBodyLen = streamingMessage?.body.length ?? 0;
  useEffect(() => {
    if (!streamingMessage || !stickRef.current) return;
    const el = parentRef.current;
    if (el) {
      el.scrollTop = el.scrollHeight;
    }
  }, [streamBodyLen, streamingMessage?.id]);
  ...
  return (
    <div className="flex-1 flex flex-col bg-background">
      ...
      <div ref={parentRef} onScroll={handleScroll} className="flex-1 overflow-y-auto px-4 py-3">
        ...
        {/* Streaming bubble — rendered OUTSIDE the virtualizer so token
            updates only re-render this one component, not the entire list. */}
        {streamingMessage && (
          <div className="mt-0.5">
            <MemoMessageBubble msg={{
              id: streamingMessage.id,
              channelId: "roundtable",
              from: streamingMessage.from,
              to: "all",
              body: streamingMessage.body,
              timestamp: streamingMessage.ts,
              streaming: true as const,
              streamCreatedAt: streamingMessage.createdAt,
            }} />
          </div>
        )}
      </div>
