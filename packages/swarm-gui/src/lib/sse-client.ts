...
export function setActiveSSESession(name: string | null): void {
  ...
  activeSession = name;
  // Deliberate session switch — suppress the connection notification so
  // the UI doesn't flash "Reconnecting" between disconnect and the next onopen.
  sseClient.disconnect(false);
