/**
 * Bun `--preload` shim for the stp dev launcher (`scripts/stp`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the stp CLI: Bun reads
 ...
 * import-time snapshots (e.g. `getProjectDir()` in `@satopi/pi-utils/dirs`).
 */
const launchCwd = process.env.STP_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.STP_LAUNCH_CWD;
	try {
