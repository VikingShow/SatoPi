...
import { getAutoresearchDbPath, getAutoresearchProjectDir, logger } from "@satopi/pi-utils";
 ...
 *
 * Used to key per-project autoresearch state under `~/.stp/autoresearch/`.
...
async function resolveAutoresearchPaths(cwd: string): Promise<{ dbPath: string; projectDir: string }> {
	const override = process.env.STP_AUTORESEARCH_DB_DIR;
