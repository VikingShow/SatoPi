...
import { $which, logger, Snowflake } from "@satopi/pi-utils";
...
export async function startRecording(outputPath: string): Promise<RecordingHandle> {
	...
	if (recorders.length === 0) {
		throw new Error("No audio recorder available — run `stp setup speech`");
	}
	...
	throw new Error(`No audio recorder could start — run \`stp setup speech\`.\n${failures.join("\n")}`);
}
