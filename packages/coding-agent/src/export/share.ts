...
import type { AgentMessage, AgentState } from "@satopi/pi-agent-core";
import type { AssistantMessage, ImageContent, TextContent } from "@satopi/pi-ai";
import { $which, logger } from "@satopi/pi-utils";
...
async function tryCreateGist(sealed: Uint8Array): Promise<{ id: string; url: string } | null> {
	...
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "stp-share-"));
	try {
