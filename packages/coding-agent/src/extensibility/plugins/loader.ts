...
/**
 * Load project-local plugin overrides (checks .stp and .pi directories).
 */
 ...
 * (`getPluginsDir(home)`) and, when a project anchor (`.stp/` or `.git/`)
 * exists at or above `cwd`, the project root
 * (`<projectAnchor>/.stp/plugins`). Each root contributes the union of its
