...
import { $which, isRecord, logger, pathIsWithin, type WhichOptions } from "@satopi/pi-utils";
...
function getConfigSources(cwd: string): ConfigSource[] {
	...
	// Project config directories (.stp/, .pi/, .claude/)
	...
	// User config directories (~/.stp/agent/, ~/.pi/agent/, ~/.claude/)
 ...
 * 2. Project config dirs: .stp/lsp.*, .pi/lsp.*, .claude/lsp.* (+ hidden variants)
 * 3. User config dirs: ~/.stp/agent/lsp.*, ~/.pi/agent/lsp.*, ~/.claude/lsp.* (+ hidden variants)
