...
import type * as MnemopiNs from "@satopi/pi-mnemopi";
import type { Mnemopi, RecallResult } from "@satopi/pi-mnemopi";
import type * as MnemopiCoreNs from "@satopi/pi-mnemopi/core";
import type { LocalModelInitializer } from "@satopi/pi-mnemopi/core";
...
/**
 * Lazily load `@satopi/pi-mnemopi` (memoized) and route fastembed loads
...
export async function loadMnemopi(): Promise<typeof MnemopiNs> {
	if (!mnemopiMod) {
		mnemopiMod = await import("@satopi/pi-mnemopi");
		installLocalModelInitializer(mnemopiMod.setLocalModelInitializer);
	}
	return mnemopiMod;
}

/** Lazily load `@satopi/pi-mnemopi/core` (memoized). */
export async function loadMnemopiCore(): Promise<typeof MnemopiCoreNs> {
	if (!mnemopiCoreMod) {
		mnemopiCoreMod = await import("@satopi/pi-mnemopi/core");
