...
import { CONFIG_DIR_NAME, getAgentDir, isEnoent, logger, prompt } from "@satopi/pi-utils";
 ...
 * `.stp/<F>` for each given filename — and return the readable candidates with
...
export async function collectConfigCandidates(
	...
	// 1. User level: ~/.stp/<F> (or active profile agent dir)
	if (resolvedAgentDir) {
	...
	// 2. Project levels (both standalone and native config .stp/): walk up from cwd to repoRoot / home
	let current = cwd;
	while (true) {
		for (const filename of filenames) {
...
/**
 * Discover and load WATCHDOG.md files walking up from cwd, project ${CONFIG_DIR_NAME}/ folder, and user agent dir.
