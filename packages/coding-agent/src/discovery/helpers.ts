...
import { FileType, glob } from "@satopi/pi-natives";
import {
	CONFIG_DIR_NAME,
	getAgentDir,
	getConfigDirName,
 ...
 *
 * Walk order:
 * 1. Walk up from `cwd` looking for the nearest directory containing `.stp/`.
 *    The first match returns `<dir>/.stp/plugins/installed_plugins.json`.
 * 2. If no `.stp/` is found, rescan from `cwd` upward looking for `.git`.
 *    The git root is used as an anchor: `<gitRoot>/.stp/plugins/installed_plugins.json`.
...
export async function resolveActiveProjectRegistryPath(cwd: string): Promise<string | null> {
	// Pass 1: walk up looking for an existing .stp/ directory (nearest wins).
	// Stop before os.homedir() — ~/.stp/ is the user-level config dir, not a project root.
...
/**
 * Like resolveActiveProjectRegistryPath, but falls back to `<cwd>/.stp/plugins/installed_plugins.json`
 * when no project anchor (.stp/ or .git/) is found.
 *
 * Use this when the caller accepts an explicit --scope project so that installing into a freshly
 * bootstrapped directory (no .stp/ or .git/ yet) works: writeInstalledPluginsRegistry auto-creates
 ...
 * Reads ~/.claude/plugins/installed_plugins.json and ~/.stp/plugins/installed_plugins.json,
...
export async function listClaudePluginRoots(
	...
	// Loaded from the nearest .stp/plugins/installed_plugins.json relative to cwd.
