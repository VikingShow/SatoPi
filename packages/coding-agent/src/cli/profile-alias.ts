...
function validateAliasName(aliasName: string, shell: ProfileAliasShell): string {
	...
	if (normalized.toLowerCase() === "stp") {
		throw new Error('Invalid alias "stp". Refusing to shadow the base stp command.');
	}
...
function renderAliasBlock(
	...
	const start = `# >>> stp profile alias: ${aliasName} >>>`;
	const end = `# <<< stp profile alias: ${aliasName} <<<`;
	let body: string;
	switch (shell) {
		case "fish":
			body = [
				`function ${aliasName} --wraps stp --description 'STP profile ${profile}'`,
