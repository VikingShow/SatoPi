/**
 * `stp models` — list, search, and refresh available models.
 *
 * Subcommands:
 ...
 *   then list. This is the supported replacement for `rm -rf ~/.stp/models.db`
 ...
 */
import type { Api, Effort, Model } from "@satopi/pi-ai";
 ...
 * name doubles as an `stp models <provider>` shortcut.
 */
...
/** `stp models ls`/`find`: provider-grouped listing (one box table per provider). */
...
/**
 * Entry point for the standalone `stp models` command: bootstraps auth storage,
...
export async function runModelsCommand(command: ModelsCommandArgs): Promise<void> {
	...
	if (action === "find" && (!pattern || pattern.trim().length === 0)) {
		process.stderr.write("`stp models find` requires a search substring, e.g. `stp models find minimax`\n");
		process.exitCode = 1;
		return;
	}
