 ...
 */
import type { ArgDescriptor, CliConfig, CommandCtor, FlagDescriptor } from "@satopi/pi-utils/cli";
...
function bashValueBranch(bin: string, v: ValueSource): string {
	...
	switch (v.kind) {
		...
		case "list":
			return `${fn}_comma "${bashWords(v.values)}"; return 0`;
		case "models":
			return v.multiple
				? `${fn}_comma "$(command ${bin} __complete models 2>/dev/null | cut -f1)"; return 0`
...
function zshAction(v: ValueSource, fn: string): string {
	switch (v.kind) {
		case "flag":
			return "";
		case "value":
		...
		case "list":
			return `:value:${fn}_tools`;
		case "models":
			return v.multiple ? `:models:${fn}_models_list` : `:model:${fn}_call models`;
		case "sessions":
			return `:session:${fn}_call sessions`;
		case "file":
			return ":file:_files";
		case "dir":
			return ":dir:_files -/";
...
function zshFlagSpec(f: CompletionFlag, fn: string): string {
	const body = `[${zshDesc(f.description)}]${zshAction(f.value, fn)}`;
...
function generateZsh(spec: CompletionSpec): string {
	const { bin } = spec;
	const fn = `_${bin}`;
	// The `:value:${fn}_tools` action references this helper; bake its candidates
...
${cmdRows}
	...
	const aliasArms = spec.commands
		.map(c => `\t\t\t${commandTokens(c).join("|")}) ${fn}_cmd_${bashFn(c.name)} ;;`)
		.join("\n");
	const rootSpecs = [
		"'(-h --help)'{-h,--help}'[Show help]'",
...
# Works both ways: autoloaded from $fpath (file named ${fn}) or eval'd from a
# startup file. When autoloaded, funcstack[1] is ${fn} and we invoke it; when
...
function generateFish(spec: CompletionSpec): string {
	...
	lines.push(`function __fish_${bin}_no_subcommand`);
	...
	const rootCond = `__fish_${bin}_no_subcommand`;
