/**
 * CLI handler for `stp worktree` — list and clean up agent-managed worktrees.
 *
 * Layout under `~/.stp/wt/`:
 *
...
import { getWorktreesDir, isEnoent } from "@satopi/pi-utils";
...
export interface WorktreeEntry {
	/** Absolute path to the worktree dir (or stray container) under `~/.stp/wt/`. */
	path: string;
	...
	branch?: string;
	/** When set, the entry is unhealthy and `stp worktree clear` will remove it. */
	orphanReason?: string;
}
...
async function scanWorktrees(): Promise<WorktreeEntry[]> {
	...
	for (const name of topLevel) {
		...
		// Legacy nesting: ~/.stp/wt/<encoded-project>/<branch-or-id>
		let children: string[];
		try {
