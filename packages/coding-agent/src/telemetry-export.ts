 ...
 *
 * oh-my-pi's agent core (`@satopi/pi-agent-core`) emits OpenTelemetry GenAI
 ...
 * spawns `stp` as a child process) had no way to collect omp's internal traces.
 *
...
import { logger, postmortem } from "@satopi/pi-utils";

/**
 * Periodic flush interval. A long-lived `stp` process (the ACP server is
