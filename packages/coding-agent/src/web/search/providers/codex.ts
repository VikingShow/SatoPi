...
import { decodeJwt } from "@satopi/pi-ai/oauth/openai-codex";
import { getBundledModels } from "@satopi/pi-catalog/models";
import { $env, readSseJson } from "@satopi/pi-utils";
...
export async function searchCodex(params: SearchParams): Promise<SearchResponse> {
	...
	if (!seed) {
		throw new Error(
			"No Codex OAuth credentials found. Login with 'stp /login openai-codex' to enable Codex web search.",
		);
	}
