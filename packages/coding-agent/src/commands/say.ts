...
export default class Say extends Command {
	...
	static examples = [
		'stp say "hello world"',
		"stp say --file notes.md --voice bm_fable",
		'stp say "hello world" --out /tmp/hello.wav',
	];
	...
	#synthesisFailed(model: string): void {
		process.stderr.write(
			chalk.red(
				`error: could not synthesize with local TTS model "${model}". Run \`stp setup speech\` to install it.\n`,
			),
		);
	}
}
