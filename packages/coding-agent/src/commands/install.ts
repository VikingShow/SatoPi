/**
 * `stp install <target>` — top-level convenience over `stp plugin install` /
 * `stp plugin link`.
 *
 * The docs (omp.sh/docs/extension-authoring) advertise
 *
 *   stp install ./my-extension
 *
...
/**
 * Heuristic used to decide whether `stp install <target>` should `link` a
...
export default class Install extends Command {
	...
	async run(): Promise<void> {
		...
		if (targets.length === 0) {
			process.stderr.write("Usage: stp install <path | npm-spec | name@marketplace> [...]\n");
			process.exit(1);
		}
