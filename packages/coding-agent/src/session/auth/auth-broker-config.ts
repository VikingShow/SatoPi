 ...
 * `@satopi/pi-ai/auth-broker/discover` that preserves the process-lifetime
 ...
 *   2. `auth.broker.url` / `auth.broker.token` in `~/.stp/agent/config.yml`
 *      (hidden from the settings UI; `!command` resolution supported).
 *   3. Token file `~/.stp/auth-broker.token` (paired with URL from env or config).
 *
 ...
 * that flip `STP_AUTH_BROKER_*` between cases still observe the change, while
...
export function resolveAuthBrokerConfig(): Promise<AuthBrokerClientConfig | null> {
	const key = `${process.env.STP_AUTH_BROKER_URL ?? ""}\u0000${process.env.STP_AUTH_BROKER_TOKEN ?? ""}\u0000${getAgentDir()}`;
