/**
 * session/store/session-entry-index.ts — Derived index views over a session's
 * entry list, extracted from `session-manager.ts` (stage 2b).
 *
 * Maintains id lookup, parent→children adjacency, the resolved label map, the
 * active leaf, and running usage totals in lockstep with the manager's
 * `#entries` so reads stay O(1)/O(children) instead of rescanning the journal.
 */

import type { Usage } from "@satopi/pi-ai";
import { type SessionEntry, type SessionTreeNode, type UsageStatistics } from "../session-entries";
