...
function serializeRoleYaml(role: RoleAsset): string {
	...
	if (role.skills && role.skills.length > 0) {
		lines.push("skills:");
		for (const s of role.skills) lines.push(`  - "${s}"`);
	}
	if (role.mcp_servers && role.mcp_servers.length > 0) {
		lines.push("mcp_servers:");
		for (const m of role.mcp_servers) lines.push(`  - "${m}"`);
	}
	if (role.model) {
	...
	return [
		{
			...
			tools: ["read_file", "search_file", "search_content", "write_to_file", "list_dir"],
		...
		},
		{
			...
			tools: ["read_file", "write_to_file", "execute_command", "search_content", "search_file", "list_dir"],
		...
		},
		{
			...
			tools: ["read_file", "write_to_file", "search_content", "search_file", "list_dir"],
		...
		},
		{
			...
			tools: ["read_file", "search_content", "search_file", "list_dir"],
			tags: ["review", "quality", "security", "code-review"],
		},
		{
			id: "devops-engineer",
			name: "DevOps Engineer",
			description: "Manages deployment, CI/CD pipelines, infrastructure configuration, and operational concerns.",
			author: "swarm",
			prompts: {
				system:
		...
		},
		{
			...
			description:
				"Reviews output for structural integrity: API contracts, module boundaries, dependency direction.",
			author: "swarm",
			prompts: {
				system:
