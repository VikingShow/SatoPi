...
describe("IrcBus - sendToGroup", () => {
	...
	it("returns a map of receipts keyed by agent id", async () => {
		...
		const result = await bus.sendToGroup(["agent-a", "agent-b"], { from: "caller", body: "hello" });
...
describe("IrcBus - collectResponses", () => {
	...
	it("respects timeout (does not hang)", async () => {
		...
		const result = await bus.collectResponses("caller", ["ghost"], { from: "caller", body: "quick" }, {}, 500);
