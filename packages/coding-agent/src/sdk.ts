...
import type { Context, CredentialDisabledEvent, Message, Model, SimpleStreamOptions } from "@satopi/pi-ai";
import type { Dialect } from "@satopi/pi-ai/dialect";
import {
	getOpenAICodexTransportDetails,
	prewarmOpenAICodexResponses,
} from "@satopi/pi-ai/providers/openai-codex-responses";
import { FALLBACK_DIALECT, preferredDialect } from "@satopi/pi-catalog/identity";
import type { Component } from "@satopi/pi-tui";
import { $env, $flag, getAgentDir, getProjectDir, logger, postmortem, prompt, Snowflake } from "@satopi/pi-utils";
...
export interface CreateAgentSessionOptions {
	...
	cwd?: string;
	/** Global config directory. Default: ~/.stp/agent */
	agentDir?: string;
	...
	/**
	 * Pre-discovered custom-tool source paths from `.stp/tools/`, `.claude/tools/`,
	...
	/** Prompt templates. Default: discovered from cwd/.stp/prompts/ + agentDir/prompts/ */
	...
	/** Active MMD content string (generated by OffloadManager/MermaidSynthesizer). */
	activeMmd?: string;
	/** Offload manager for L3 compact context summaries. */
	offloadManager?: IOffloadManager;
	/** Context window size for compaction threshold (tokens). */
	contextWindow?: number;
	/** Pre-computed injected context messages from ContextPipeline. */
 ...
 *
 * // With explicit model
 * import { getModel } from '@satopi/pi-ai';
 * const { session } = await createAgentSession({
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	const markEnvironment = options.markEnvironment ?? new MarkEnvironment();
	const agentKind =
		options.agentKind ??
	...
	try {
		...
		// Discover custom tools from `.stp/tools/`, `.claude/tools/`, plugins, etc.
		...
		// cache that `stp models find` uses before the online refresh continues in
		// the background.
		...
		const transformContext = pipelineTransform
			? async (messages: AgentMessage[], signal?: AbortSignal) => {
					...
					const markCtx = markEnvironment.getContextForAgent(options.agentId ?? "main");
					if (markCtx) {
						result = [
							{
								role: "user",
