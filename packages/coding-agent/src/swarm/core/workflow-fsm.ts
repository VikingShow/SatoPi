/**
 * WorkflowFSM — Unified phase state machine for SatoPi swarm v3.
 *
 * ## Role
 *
 * WorkflowFSM is the single authority for workflow phase transitions. It replaces
 * the scattered phase tracking (SwarmStateMachine, ScriptManager #phase/#busy,
 * StateTracker Chapter, and CurtainRunner) with a unified FSM that delegates
 * persistence to StateTracker and event logging to ActivityLogger.
 *
 * ## Lifecycle
 *
 ...
 * ## Integration
 *
 * This is a drop-in replacement for the existing SwarmStateMachine. It provides
 * the same `transition()` / `force()` / `phase` API, plus additional methods
...
const ACTIVE_PHASES: Set<Chapter> = new Set([
  "script",
  "script-debate",
  "stage",
  "curtain",
...
export type PhaseChangeListener = (event: {
  from: Chapter;
  to: Chapter;
  meta?: TransitionMeta;
}) => void;
