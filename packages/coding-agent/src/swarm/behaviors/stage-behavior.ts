...
export class StageBehavior implements PhaseBehavior {
  ...
  async enter(ctx: PhaseContext): Promise<PhaseEnterResult> {
    ...
    const channel = ctx.commBus.groupChannel(
      "swarm",
      ["human", ...agentIds],
      ctx.activityLogger,
    );
    ...
    const specs = roles.map((role, i) => {
      ...
      const initialTask = roleTasks.length > 0
        ? roleTasks.map((t) => t.title).join("; ")
        : "Execute build tasks as assigned";
    ...
    switch (event.status) {
      case "completed": {
        ...
        logger.info("[StageBehavior] Agent completed", {
          agentId: event.agentId,
          remainingActive:
            this.#agents.size - this.#completedAgents.size,
        });
        break;
      }
  ...
  async checkCompletion(_ctx: PhaseContext): Promise<PhaseCompletion | null> {
    ...
    const allFinished =
      ...
      [...this.#agents.values()].every(
        (t) =>
          t.handle.status === "completed" ||
          t.handle.status === "failed" ||
          t.handle.status === "aborted",
      );

    if (this.#agents.size > 0 && this.#completedAgents.size === this.#agents.size) {
      // All agents have completed — check task queue too
      const tasksDone =
        !this.#taskQueue || this.#taskQueue.isAllComplete;

      if (tasksDone) {
        // Count failures
        const failures = [...this.#agents.values()].filter(
          (t) => t.hasError,
        ).length;
