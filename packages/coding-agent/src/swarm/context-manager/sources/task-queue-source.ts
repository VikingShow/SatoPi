  ...
  async build(_spec: AgentSpecLike, _base: BuildContext): Promise<ContextFragment> {
    ...
    if (readyTasks.length > 0) {
      ...
      for (const taskId of readyTasks.slice(0, 10)) {
        ...
        if (task) {
          lines.push(`    - ${task.id}: ${task.title} (role: ${task.assignedRole}, est: ${task.estimatedMinutes}m)`);
        }
      }
    ...
    if (completed.length > 0) {
      lines.push(`  Completed: ${completed.slice(0, 10).join(", ")}${completed.length > 10 ? ` ... and ${completed.length - 10} more` : ""}`);
    }
