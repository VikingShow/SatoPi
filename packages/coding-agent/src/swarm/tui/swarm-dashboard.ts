...
import { renderCommPanel, type CommMessage } from "./comm-panel";
import { renderContextPanel, type ContextPanelState } from "./context-panel";
...
function renderTwoColumn(input: DashboardInput, width: number): string[] {
  ...
  for (let i = 0; i < maxRows; i++) {
    const left = i < agentLines.length
      ? agentLines[i].padEnd(leftWidth)
      : " ".repeat(leftWidth);
    const right = i < rightCombined.length
      ? rightCombined[i]
      : "";
