...
import { PHASE_DISPLAY, SATOPI_COLORS, ansiFg, ansiBold, ansiDim } from "./theme";
...
export function renderPhaseView(state: SwarmState): string[] {
  ...
  // Time
  statusParts.push(
    `${ansiDim("Time:")} ${ansiFg(SATOPI_COLORS.text.ansi256, elapsedStr)}`
  );

  // Agent count
  const agentStr =
    runningAgents > 0
      ? `${agentCount} (${runningAgents} running)`
      : `${agentCount}`;
  statusParts.push(
    `${ansiDim("Agents:")} ${ansiFg(SATOPI_COLORS.text.ansi256, agentStr)}`
  );
  ...
  if (state.loopIteration !== undefined && state.loopIteration > 0) {
    statusParts.push(
      `${ansiDim("Iteration:")} ${ansiFg(SATOPI_COLORS.text.ansi256, String(state.loopIteration))}`
    );
  }
  ...
  if (state.roundtablePhase) {
    statusParts.push(
      `${ansiDim("Step:")} ${ansiFg(SATOPI_COLORS.purple.ansi256, state.roundtablePhase)}`
    );
  }
