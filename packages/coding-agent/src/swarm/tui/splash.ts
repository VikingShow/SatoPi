import { ansiBold, ansiDim, ansiFg, PI_LOGO_ASCII, SATOPI_COLORS } from "./theme";
...
export function renderSplash(width: number = 80): string[] {
	...
	lines.push(
		ansiFg(borderColor, "║") + nameLine + " ".repeat(Math.max(0, innerW - (namePad + 8))) + ansiFg(borderColor, "║"),
	);
