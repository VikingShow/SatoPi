 ...
 */
import { SATOPI_COLORS, ansiFg, ansiBold, ansiDim } from "./theme";
...
export function renderAgentPanel(state: SwarmState | null | undefined, maxWidth: number): string[] {
  ...
  if (reviewer) {
    ...
    if (verdict) {
      ...
      if (verdictBudget >= 5) {
        verdictDisplay = verdict.length > verdictBudget
          ? verdict.slice(0, Math.max(1, verdictBudget - 3)) + "..."
          : verdict;
      } else {
...
function _formatAgentLine(
  ...
  if (visibleLength(line) > innerWidth) {
    ...
    if (agent.status === "failed" && statusBudget > 10) {
      ...
      if (errBudget >= 3) {
        const truncated = err.length > errBudget
          ? err.slice(0, Math.max(1, errBudget - 3)) + "..."
          : err;
      ...
      const truncated = label.length > statusBudget
        ? label.slice(0, Math.max(1, statusBudget - 3)) + "..."
        : label;
...
function _padLine(content: string, width: number): string {
  ...
  if (visibleLength(content) > maxContent) {
    ...
    for (const ch of content) {
      if (ch === "\x1b") { inAnsi = true; truncated += ch; continue; }
      if (inAnsi) { truncated += ch; if (ch === "m") inAnsi = false; continue; }
