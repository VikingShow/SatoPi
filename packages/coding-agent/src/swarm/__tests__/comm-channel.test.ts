...
import { runRoundtable, jaccardSimilarity, tokenize } from "../comm-bus/roundtable";
import { runVote, parseVote } from "../comm-bus/vote";
...
describe("jaccardSimilarity", () => {
  ...
  test("convergence threshold — highly similar texts exceed 0.85", () => {
    const a = "I think the architecture should use a microservices approach with event-driven communication between services";
    const b = "I think the architecture should use microservices with event-driven communication between the services";
...
describe("runRoundtable (ghost agents)", () => {
  ...
  test("ghost agents run full rounds, no responses collected", async () => {
    const result = await runRoundtable(
      ...
      ["ghost-x", "ghost-y"],
      "architecture debate",
      { rounds: 2, timeoutMs: 50, convergenceThreshold: 0.85, convergenceStreak: 2 },
    );
    expect(result.responses).toEqual([]);
    expect(result.finalPositions).toEqual([]);
...
describe("CommChannel", () => {
  ...
  test("sendToGroup targets specific subset", async () => {
    ...
    await expect(
      ch.sendToGroup("a1", "secret", ["a2", "a3"]),
  ...
  test("interrupt sends steering directive", async () => {
    ...
    await expect(
      ch.interrupt("human", "a1", "Please reconsider"),
    ).resolves.toBeUndefined();
  });
...
describe("CommBus", () => {
  ...
  test("receiveFromHuman targets a specific agent", async () => {
    ...
    await expect(
      cb.receiveFromHuman("hello worker", "worker-1"),
    ).resolves.toBeUndefined();
  });
