...
import { RoleProvider, type ResolvedRole } from "../agent-runtime/role-provider";
...
describe("AgentLauncher", () => {
  ...
  function makeLaunchContext(overrides?: Partial<LaunchContext>): LaunchContext {
    ...
    return {
      ...
      modelRegistry: {
        getAvailable: () => [
          { id: "test-model", provider: "test", supportsTools: true },
        ],
  ...
  test("throws when no models available", async () => {
    ...
    await expect(launcher.launch(ctx)).rejects.toThrow(
      /No available model/,
    );
  });
...
describe("AgentRuntime", () => {
  ...
  describe("spawnRoundtable()", () => {
    test("returns a stub result (not yet implemented)", async () => {
      const runtime = new AgentRuntime({
        roleProvider,
        contextPipeline,
        launcher,
      ...
      const config: RoundtableConfig = { rounds: 3 };
      const result = await runtime.spawnRoundtable(
        [makeSpec({ id: "a1" }), makeSpec({ id: "a2" })],
        config,
      );
...
describe("Error handling", () => {
  test("AgentRuntime.spawnOne propagates RoleProvider errors", async () => {
    ...
    await expect(
      runtime.spawn([makeSpec({ id: "bad-agent" })]),
    ).rejects.toThrow("Role DB offline");
  });
  ...
  test("AgentRuntime.spawnOne propagates ContextPipeline errors", async () => {
    ...
    const modelRegistry = {
      getAvailable: () => [
        { id: "test", provider: "test", supportsTools: true },
      ],
      resolver: async () => undefined,
    } as unknown as ModelRegistry;
