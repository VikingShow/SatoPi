...
export async function generatePlanningPrompt(config: ScriptConfig, experienceStore?: ExperienceStore): Promise<string> {
	...
	const sections = [
		...
		"## Process",
		"",
		"⚠️  These are RULES for a multi-turn dialogue. They are NOT steps to complete",
		"    in one response. You will go through MANY turns with the human.",
		"",
		"⚠️  ONE ASPECT PER TURN. Pick ONE area (goals, scope, constraints, tech",
		"    stack, acceptance criteria). Ask 1-3 clarifying questions about it.",
		"    Your response ENDS after your questions. The human answers next turn.",
		"",
		"⚠️  CONFIRM BEFORE WRITING. Only after the human confirms a decision,",
		`    write ONLY that section to \`${planPath}\` (append, never overwrite).`,
		"    Use [DRAFT] markers for sections still under discussion.",
		"",
		"⚠️  NEVER GUESS. If information is missing or vague, ASK. Do not fill",
		"    gaps with assumptions. 5 questions are better than 1 wrong assumption.",
		"",
		"⚠️  FINAL PLAN SECTIONS (fill in as confirmed):",
		"    - What to build / achieve",
		"    - Constraints and non-goals",
		"    - Acceptance criteria",
		"    - Structured todo tasks (with dependencies)",
		"    - Agent-hour estimate: ~0.5h/task + ~0.25h/unique file,",
		"      with multipliers: safety-critical 1.5×, cross-pkg 1.3×, cross-lang 1.2×",
		`    - Recommended agents (default: ${defaultAgents}, one agent ≈ 4h focused work)`,
		"",
		"⚠️  SIGNAL READINESS. When ALL sections confirmed, remove [DRAFT] markers",
		`    and say: "The plan is complete and ready for debate."`,
		"",
		"## Guidelines",
		"",
...
export async function runPlanDebate(
	...
	const table = new DebateRoundtable({
		agentCount: debateConfig.agentCount,
