...
import type { AssistantMessage, Model } from "@satopi/pi-ai";
import { getBlobsDir, isEnoent, logger, VERSION } from "@satopi/pi-utils";
...
import { isSilentAbort, SKILL_PROMPT_MESSAGE_TYPE, USER_INTERRUPT_LABEL } from "../../session/message/messages";
import { BlobStore, resolveImageDataSync } from "../../session/store/blob-store";
import type { UsageStatistics } from "../../session/store/session-entries";
import type { SessionInfo as StoredSessionInfo } from "../../session/store/session-listing";
	...
	async initialize(params: InitializeRequest): Promise<InitializeResponse> {
		...
		const authMethods: AuthMethod[] = [
			{
				...
				description: "Authenticate via the provider keys/OAuth state already configured under ~/.stp.",
			},
		];
		if (params.clientCapabilities?.auth?.terminal === true) {
			authMethods.push({
				type: "terminal",
				id: "terminal",
				name: "Set up SatoPi in terminal",
