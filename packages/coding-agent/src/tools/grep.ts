...
import { formatCodeFrameLine } from "./render-utils";
...
async function resolveArchiveSearchPaths(
	...
	for (let idx = 0; idx < pathSpecs.length; idx++) {
		...
		if (!tempDir) {
			tempDir = await mkdtemp(path.join(tmpdir(), "stp-search-archive-"));
		}
...
const STP_ROOT_URL_RE = /^stp:\/\/(?:\/?|docs\/?)$/i;
...
async function searchVirtualResources(
	...
	const dir = await mkdtemp(path.join(tmpdir(), "stp-search-virtual-"));
	try {
...
async function expandVirtualInternalResource(
	...
	if (STP_ROOT_URL_RE.test(rawPath)) {
		...
		for (const completion of completions) {
			...
			const docUrl = `stp://${completion.value}`;
...
// TUI renderer extracted to ./grep/render.ts (stage 3 split).
export { grepToolRenderer } from "./grep/render";
