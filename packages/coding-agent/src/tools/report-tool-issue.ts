...
import type { FetchImpl } from "@satopi/pi-ai";
import { $env, $flag, APP_NAME, getAutoQaDbDir, getInstallId, logger, VERSION } from "@satopi/pi-utils";
 ...
 * `~/.stp/agent/autoqa.db`. Idempotently runs schema creation, the
 * `pushed`-column migration, and index setup so every consumer — tool
 * execute path, manual `stp grievances push`, future debug scripts —
 ...
 *
 * Exported because the `stp grievances` CLI handlers need the migrated
...
/**
 * Optional per-flush controls. Used by `stp grievances push` to surface
...
function resolvePushConfig(settings: Settings | undefined, bypassConsent: boolean): PushConfig | null {
	...
	// covers explicit user-driven pushes (`stp grievances push`) where the
...
export function createReportToolIssueTool(session: ToolSession, activeBuiltinNames: readonly string[] = []): AgentTool {
	...
	return {
		...
		async execute(_toolCallId, rawParams) {
			...
			// at ~/.stp/agent/autoqa.db regardless of consent — they always
			// own their local data and can inspect or wipe it via `stp grievances`.
			...
			try {
				...
				if (db) {
					...
					//      `stp grievances push` or a future consent change.
