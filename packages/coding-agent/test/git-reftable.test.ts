...
describe.skipIf(!supportsReftable)("git reftable support", () => {
	...
	beforeAll(async () => {
		...
		sharedRepoDir = await fs.mkdtemp(path.join(os.tmpdir(), "stp-reftable-"));
		...
		worktreeDir = await fs.mkdtemp(path.join(os.tmpdir(), "stp-reftable-wt-"));
		...
		configRepoDir = await fs.mkdtemp(path.join(os.tmpdir(), "stp-reftable-cfg-"));
