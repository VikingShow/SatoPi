...
import type { IsolationHandle, WorktreeBaseline } from "@satopi/pi-coding-agent/task/worktree";
...
import "@satopi/pi-coding-agent/tools/yield";
...
describe("subagent LSP availability", () => {
	...
	it("opens isolated persisted subagent sessions with the worktree cwd", async () => {
		...
		mockIsolation();
		const { getOptions } = mockCreateAgentSession();
		const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "stp-isolated-session-cwd-"));
		try {
