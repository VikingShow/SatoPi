 ...
 * profile. A profile relocates the agent directory to ~/.stp/profiles/<name>/agent;
 * the native provider used to read user config (commands, skills, rules, etc.)
 * from the literal home (~/.stp/agent) via `ctx.home`, leaking the default
 ...
 * (ctx.home + ".stp/agent") points at the tempdir decoys below; without the fix
