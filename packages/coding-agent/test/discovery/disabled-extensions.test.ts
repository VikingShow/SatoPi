...
describe("disabledExtensions runtime filtering", () => {
	...
	beforeEach(async () => {
		...
		setAgentDir(path.join(tempHomeDir, ".stp", "agent"));
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "stp-disabled-ext-"));
		await fs.mkdir(path.join(tempDir, ".stp"), { recursive: true });
		await fs.writeFile(path.join(tempDir, ".stp", "AGENTS.md"), "# project instructions\n");
	...
	afterEach(async () => {
		...
		restoreEnvValue("STP_PROFILE", originalStpProfileEnv);
