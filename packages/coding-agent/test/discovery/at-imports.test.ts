...
import { expandAtImports, MAX_AT_IMPORT_DEPTH } from "@satopi/pi-coding-agent/discovery/at-imports";
...
describe("expandAtImports", () => {
	...
	beforeEach(async () => {
		tmp = await fs.mkdtemp(path.join(os.tmpdir(), "stp-at-import-"));
	});
	...
	test("resolves ~/path against the home override", async () => {
		const fakeHome = await fs.mkdtemp(path.join(os.tmpdir(), "stp-at-home-"));
		try {
