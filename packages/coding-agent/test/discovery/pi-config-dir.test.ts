...
import { getConfigDirs } from "@satopi/pi-coding-agent/config";
import { getUserPath } from "@satopi/pi-coding-agent/discovery/helpers";
...
describe("PI_CONFIG_DIR", () => {
	...
	test("getUserPath resolves the native user scope via getAgentDir (profile-aware)", () => {
		...
		// The old behavior joined ctx.home + ".stp/agent" and leaked the default
	...
	test("getConfigDirs respects PI_CONFIG_DIR for user base", () => {
		process.env.PI_CONFIG_DIR = ".config/stp";
		const result = getConfigDirs("commands", { project: false });
		const expected = path.resolve(path.join(os.homedir(), ".config/stp", "agent", "commands"));
		expect(result[0]).toEqual({ path: expected, source: ".stp", level: "user" });
	});
});
