...
import { type Rule, ruleCapability } from "@satopi/pi-coding-agent/capability/rule";
import type { LoadContext } from "@satopi/pi-coding-agent/capability/types";
...
beforeEach(() => {
	...
	setAgentDir(path.join(home, ".stp", "agent"));
});
...
test("user ~/.stp/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".stp", "agent", "RULES.md"),
...
test("project .stp/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".stp", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".stp", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".stp", "RULES.md"));
});

test("user and project sticky RULES.md both survive public capability dedup", async () => {
	const userRulesPath = path.join(home, ".stp", "agent", "RULES.md");
	const projectRulesPath = path.join(project, ".stp", "RULES.md");
...
test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".stp", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	...
	writeFile(path.join(home, ".stp", "agent", "rules", "other.md"), "# Unrelated rule\n");
