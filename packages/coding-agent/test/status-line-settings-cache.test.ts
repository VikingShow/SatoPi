...
import { StatusLineComponent, type StatusLineSettings } from "@satopi/pi-coding-agent/modes/components/status-line";
import { STATUS_LINE_PRESETS } from "@satopi/pi-coding-agent/modes/components/status-line/presets";
import { initTheme } from "@satopi/pi-coding-agent/modes/theme/theme";
import * as git from "@satopi/pi-coding-agent/utils/git";
import { removeSyncWithRetries, setProjectDir } from "@satopi/pi-utils";
...
beforeEach(async () => {
	...
	projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "stp-status-line-settings-cache-"));
