...
import { SPINNER_GLYPH_ADVANCE_MS, sharedSpinnerFrame } from "@satopi/pi-coding-agent/modes/components/tool-execution";
import { getThemeByName } from "@satopi/pi-coding-agent/modes/theme/theme";
import { getConfigRootDir, getCustomThemesDir, removeWithRetries, setAgentDir } from "@satopi/pi-utils";
...
describe("theme symbols.spinnerFrames", () => {
	beforeEach(async () => {
		tmpAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "stp-spinner-frames-"));
