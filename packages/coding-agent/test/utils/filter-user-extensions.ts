...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@satopi/pi-utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.stp/`), plus
//    the legacy non-XDG `~/.stp/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.stp/agent` or `$PI_CODING_AGENT_DIR`)
...
// symlinks `~/.stp/agent/extensions/foo.ts` to a dotfiles repo would see the
