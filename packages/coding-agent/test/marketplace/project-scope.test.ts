...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("walk-up finds nearest .stp/ directory", async () => {
		...
		// Resolver must climb from cwd → sub → tmpDir and find .stp/ there.
		fs.mkdirSync(path.join(tmpDir, ".stp"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".stp", "plugins", "installed_plugins.json"));
	});
	...
	it("walk-up stops at the nearest .stp/ — does not skip to a more distant one", async () => {
		...
		fs.mkdirSync(path.join(tmpDir, ".stp"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".stp"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".stp", "plugins", "installed_plugins.json"));
	});
	...
	it("falls back to .git root when no .stp/ exists", async () => {
		...
		// Bound the walk at os.tmpdir() so a stray .stp/ above the fixture tree
		// (e.g. an unrelated /tmp/.stp) cannot hijack pass 1.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		try {
			const result = await resolveActiveProjectRegistryPath(cwd);
			const homeOmpPath = path.join(homeDir, ".stp", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	beforeEach(() => {
		...
		fs.mkdirSync(path.join(tmpProject, ".stp", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".stp", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".stp", "plugins", "installed_plugins.json");
	});
