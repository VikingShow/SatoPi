/**
 * Discovery integration tests for STP plugin registry reading.
 *
 * NOTE: listClaudePluginRoots() lives in discovery/helpers.ts which imports
 * @satopi/pi-natives (native Rust addon via glob). We cannot call it here.
 *
 * Instead these tests validate the structural contract that listClaudePluginRoots
 * depends on:
 *   1. OMP registry lives at path.join(home, ".stp", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".stp")
...
// Kept here to avoid importing helpers.ts (which pulls in @satopi/pi-natives).
...
// Matches getConfigDirName() — single source of truth is in @satopi/pi-utils,
// but we know the value is ".stp" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const OMP_CONFIG_DIR = ".stp";
...
let tmpHome: string;
/** ~/.stp/plugins/installed_plugins.json inside tmpHome */
let ompRegistryPath: string;

beforeEach(() => {
	tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "stp-discovery-test-"));
...
describe("OMP registry path contract", () => {
	it("OMP registry lives at home/.stp/plugins/installed_plugins.json", () => {
		// This is the path that listClaudePluginRoots reads.
		// Any change to this path must be reflected in helpers.ts.
		const expected = path.join(tmpHome, ".stp", "plugins", "installed_plugins.json");
