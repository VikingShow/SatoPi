 ...
 *   - `@satopi/swarm-extension`         ⇒ `@satopi/pi-utils`
 *   - `@plannotator/pi-extension`         ⇒ `@satopi/pi-agent-core`
 *   - `@runfusion/fusion`                 ⇒ `@satopi/pi-coding-agent/...`
 *
...
const canonicalCodingAgent = Bun.resolveSync("@satopi/pi-coding-agent", import.meta.dir);
const canonicalCodingAgentExtensions = Bun.resolveSync(
	"@satopi/pi-coding-agent/extensibility/extensions",
	import.meta.dir,
);
const canonicalUtils = Bun.resolveSync("@satopi/pi-utils", import.meta.dir);
const canonicalTui = Bun.resolveSync("@satopi/pi-tui", import.meta.dir);
// Subpath: upstream `pi-ai/oauth` re-exported `utils/oauth/index`; our pi-ai now
// exposes the same surface at the real `@satopi/pi-ai/oauth` export, so the
// legacy `@mariozechner/pi-ai/oauth` specifier canonicalizes straight to it.
const canonicalAiOauth = Bun.resolveSync("@satopi/pi-ai/oauth", import.meta.dir);
...
const CASES: readonly AliasCase[] = [
	...
	{ id: "ohmypi-utils", aliasSpecifier: "@satopi/pi-utils", canonicalPath: canonicalUtils, symbol: "logger" },
	{
		...
		aliasSpecifier: "@satopi/pi-coding-agent",
	...
	// Subpath: legacy `pi-ai/oauth` resolves to the real `@satopi/pi-ai/oauth`.
	{
