...
import { __buildLegacyPiPackageRootOverrides } from "@satopi/pi-coding-agent/extensibility/plugins/legacy-pi-compat";
...
describe("legacy pi compat compiled-mode subpath overrides (issue #3442)", () => {
	it("serves @satopi/pi-ai/oauth through the bundled virtual namespace in compiled mode", () => {
		...
		expect(overrides["@satopi/pi-ai/oauth"]).toBe("stp-legacy-pi-bundled:@satopi/pi-ai/oauth");
	});
	...
	it("expands wildcard exports for concrete on-disk targets (issue #3442 follow-up)", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@satopi/pi-ai/oauth/anthropic"]).toBe("stp-legacy-pi-bundled:@satopi/pi-ai/oauth/anthropic");
		// Sanity: the wildcard expansion also reaches deeper subroots so plugins
		// pinned to e.g. `@satopi/pi-ai/providers/openai` keep resolving.
		expect(bundledModuleKeys.has("@satopi/pi-ai/oauth/anthropic")).toBe(true);
		expect(bundledModuleKeys.has("@satopi/pi-ai/oauth/openai-codex")).toBe(true);
	});

	it("expands web search provider wildcard exports for compiled plugin imports", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		const providerKeys = [
			"@satopi/pi-coding-agent/web/search/providers/xai",
			"@satopi/pi-coding-agent/web/search/providers/tinyfish",
			"@satopi/pi-coding-agent/web/search/providers/firecrawl",
			"@satopi/pi-coding-agent/web/search/providers/duckduckgo",
		] as const;
	...
	it("does not enumerate root catch-all wildcards (./* / ./*.js)", () => {
		...
		// Concrete check: `@satopi/pi-coding-agent/cli` is NOT bundled.
		expect(bundledModuleKeys.has("@satopi/pi-coding-agent/cli")).toBe(false);
		expect(bundledModuleKeys.has("@satopi/pi-coding-agent/main")).toBe(false);
	});
	...
	it("does not bundle main-thread-unsafe worker entrypoints", () => {
		...
		expect(bundledModuleKeys.has("@satopi/pi-coding-agent/eval/js/worker-entry")).toBe(false);
	});
	...
	it("keeps pi-ai/pi-coding-agent roots routed to their compat shims in compiled mode", () => {
		...
		const overrides = __buildLegacyPiPackageRootOverrides(true, bundledModuleKeys);
		expect(overrides["@satopi/pi-ai"]).toBeDefined();
		expect(overrides["@satopi/pi-ai"]).not.toBe("stp-legacy-pi-bundled:@satopi/pi-ai/oauth");
		expect(overrides["@satopi/pi-coding-agent"]).toBeDefined();
	});

	it("does not register subpath overrides in dev/install mode", () => {
		const overrides = __buildLegacyPiPackageRootOverrides(false);
		expect(overrides).not.toHaveProperty("@satopi/pi-ai/oauth");
		expect(overrides).not.toHaveProperty("@satopi/pi-coding-agent/tools");
