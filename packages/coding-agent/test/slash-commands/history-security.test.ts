...
import { shouldSkipHistory } from "@satopi/pi-coding-agent/modes/controllers/input-controller";
...
describe("shouldSkipHistory — security filter for slash command history", () => {
	...
	it("skips /join with a link argument (carries 32-byte room key and write token)", () => {
		expect(shouldSkipHistory("/join stp://share/abc123def456...")).toBe(true);
		expect(shouldSkipHistory("/join stp:abc123def456...")).toBe(true);
	});
