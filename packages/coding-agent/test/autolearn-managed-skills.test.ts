...
import {
	...
	toSkillFrontmatter,
	writeManagedSkill,
} from "@satopi/pi-coding-agent/autolearn/managed-skills";
import { parseFrontmatter, removeWithRetries } from "@satopi/pi-utils";
...
describe("managed-skills primitives", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "stp-managed-skills-"));
	...
	describe("writeManagedSkill", () => {
		...
		it("refuses to write through a symlinked skill directory", async () => {
			...
			const outside = await fs.mkdtemp(path.join(os.tmpdir(), "stp-escape-"));
			try {
		...
		it("refuses to write when the managed-skills root itself is a symlink", async () => {
			const realRoot = await fs.mkdtemp(path.join(os.tmpdir(), "stp-realroot-"));
			try {
		...
		it("refuses to update a SKILL.md that is a symlink", async () => {
			...
			const outside = await fs.mkdtemp(path.join(os.tmpdir(), "stp-link-"));
	...
	describe("deleteManagedSkill", () => {
		...
		it("refuses to delete through a symlinked skill directory", async () => {
			const managedRoot = getManagedSkillsDir();
			await fs.mkdir(managedRoot, { recursive: true });
			const outside = await fs.mkdtemp(path.join(os.tmpdir(), "stp-deltarget-"));
