...
describe("global --profile flag", () => {
	...
	it("activates a profile inherited from STP_PROFILE at run time", async () => {
		const writeSpy = vi.spyOn(process.stdout, "write").mockImplementation(() => true);
		setProfile(undefined);
		process.env.STP_PROFILE = "work";
		delete process.env.PI_PROFILE;
	...
	it("loads profile agent .env before command modules import pi-utils env", async () => {
		...
		try {
			...
			await Bun.write(path.join(defaultAgentDir, ".env"), "STP_PROFILE_BOOTSTRAP_SENTINEL=default\n");
			await Bun.write(path.join(profileAgentDir, ".env"), "STP_PROFILE_BOOTSTRAP_SENTINEL=work\n");
			...
			await Bun.write(
				...
				[
					...
					'process.stdout.write("\\nSENTINEL=" + (Bun.env.STP_PROFILE_BOOTSTRAP_SENTINEL ?? ""));',
				].join("\n"),
			);
			...
			delete childEnv.STP_PROFILE;
			delete childEnv.PI_PROFILE;
			delete childEnv.PI_CODING_AGENT_DIR;
			delete childEnv.STP_PROFILE_BOOTSTRAP_SENTINEL;
