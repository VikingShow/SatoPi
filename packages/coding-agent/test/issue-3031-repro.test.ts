 ...
 * round-trips through `__stp_worker_mnemopi_embed`, and `SIGKILL`s the child
...
} from "@satopi/pi-coding-agent/mnemopi/embed-client";
import type {
	MnemopiEmbedWorkerInbound,
	MnemopiEmbedWorkerOutbound,
} from "@satopi/pi-coding-agent/mnemopi/embed-protocol";
...
describe("issue #3031 — mnemopi embeddings live in an isolated subprocess", () => {
	it("ping/pongs through the spawned worker subprocess and tears it down cleanly", async () => {
		...
		const script =
			'const { smokeTestMnemopiEmbedWorker } = await import("@satopi/pi-coding-agent/mnemopi/embed-client"); await smokeTestMnemopiEmbedWorker({ timeoutMs: 15000 });';
