...
import { AgentDashboard } from "@satopi/pi-coding-agent/modes/components/agent-dashboard";
import { initTheme } from "@satopi/pi-coding-agent/modes/theme/theme";
import * as discovery from "@satopi/pi-coding-agent/task/discovery";
...
async function makeTempCwd(): Promise<string> {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "stp-agent-dashboard-"));
	tempDirs.push(dir);
	return dir;
}
