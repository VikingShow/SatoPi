...
import { getAgentDir, getPluginsDir, removeSyncWithRetries, setAgentDir, TempDir } from "@satopi/pi-utils";

const currentPiCodingAgentPath = Bun.resolveSync("@satopi/pi-coding-agent", import.meta.dir);
const currentPiExtensionsPath = Bun.resolveSync("@satopi/pi-coding-agent/extensibility/extensions", import.meta.dir);
...
describe("plugin extension discovery", () => {
	...
	beforeEach(() => {
		...
		// resolves into `<tempHome>/.stp/plugins` on every platform. Two things are needed:
		...
		//    into and rm'd the developer's real `~/.stp/plugins`);
