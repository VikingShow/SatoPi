...
import { normalizeSchemaForGoogle, toolWireSchema } from "@satopi/pi-ai";
import { Settings } from "@satopi/pi-coding-agent/config/settings";
import { createTools, HIDDEN_TOOLS, type ToolSession } from "@satopi/pi-coding-agent/tools";
...
describe("tool schema validation (post-sanitization)", () => {
	...
	it("read path schema advertises local, URL, and internal URI targets", async () => {
		...
		expect(description).toContain("stp://");
