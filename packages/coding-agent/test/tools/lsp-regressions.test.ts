...
import type { AgentToolResult, RenderResultOptions } from "@satopi/pi-agent-core";
import { preloadPluginRoots } from "@satopi/pi-coding-agent/discovery/helpers";
import { LspTool } from "@satopi/pi-coding-agent/lsp";
import * as lspClient from "@satopi/pi-coding-agent/lsp/client";
import * as lspConfig from "@satopi/pi-coding-agent/lsp/config";
import { getServersForFile, type LspConfig, loadConfig } from "@satopi/pi-coding-agent/lsp/config";
import {
	applyTextEditsToString,
	applyWorkspaceEdit,
	sortAndValidateTextEdits,
} from "@satopi/pi-coding-agent/lsp/edits";
import { renderCall, renderResult } from "@satopi/pi-coding-agent/lsp/render";
import type {
	CodeAction,
	CreateFile,
	DeleteFile,
	...
	TextDocumentEdit,
	WorkspaceEdit,
} from "@satopi/pi-coding-agent/lsp/types";
import {
	applyCodeAction,
	collectGlobMatches,
	...
	resolveSymbolColumn,
	uriToFile,
} from "@satopi/pi-coding-agent/lsp/utils";
...
import { clampTimeout } from "@satopi/pi-coding-agent/tools/tool-timeouts";
import * as piUtils from "@satopi/pi-utils";
import { sanitizeText, TempDir } from "@satopi/pi-utils";
...
describe("lsp regressions", () => {
	...
	it("reload * invalidates the per-cwd config cache so newly written .stp/lsp.json is observed", async () => {
		// #3546: `getConfig` caches the first `loadConfig` result per cwd
		// permanently. Creating `.stp/lsp.json` after the first LSP call left
