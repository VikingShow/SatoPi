...
} from "@satopi/pi-coding-agent/internal-urls";
import { AgentRegistry } from "@satopi/pi-coding-agent/registry/agent-registry";
import * as sshFileTransfer from "@satopi/pi-coding-agent/ssh/file-transfer";
...
describe("GrepTool internal URL resolution", () => {
	...
	it("expands stp:// root to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# stp://tools/grep.md");
		expect(text).toContain("Grep file contents with a regex across files");
	});

	it("expands stp://docs to grep embedded documentation files", async () => {
		const session = createSession();
		const tool = new GrepTool(session);
		...
		const text = getResultText(result);
		expect(text).toContain("# stp://tools/read.md");
