...
describe("HookSelectorComponent", () => {
	...
	it("wraps outlined option text without omitting the tail", () => {
		const options = [
			"Option A: Move to OMP-native only by migrating reusable shared AI instructions into .stp/AGENTS.md, .stp/rules, .stp/skills, and .stp/agents while deliberately not creating a root .github directory.",
	...
	it("renders option descriptions as separate wrapped rows", () => {
		const options = [
			...
			{
				label: "Set up SatoPi in terminal",
				description:
