...
import { recoverOrphanedBackups } from "@satopi/pi-coding-agent/session/store/session-listing";
import { SessionManager } from "@satopi/pi-coding-agent/session/store/session-manager";
import { FileSessionStorage, MemorySessionStorage } from "@satopi/pi-coding-agent/session/store/session-storage";
...
describe("SessionManager rewrite EPERM replacement fallback", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "stp-eperm-"));
	});
...
describe("FileSessionStorage.writeTextAtomic commitGuard cleanup", () => {
	...
	beforeEach(async () => {
		sessionDir = await fsp.mkdtemp(path.join(os.tmpdir(), "stp-guard-cleanup-"));
	});
