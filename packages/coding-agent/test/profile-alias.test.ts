...
describe("profile alias installer", () => {
	it("writes a bash-compatible function that forwards subcommands through omp", async () => {
		...
		expect(result.command).toBe("stp --profile=work");
		expect(files.get("/home/me/.bashrc")).toContain("stp-work() {");
		expect(files.get("/home/me/.bashrc")).toContain('command stp --profile=work "$@"');
	});
	...
	it("writes a fish function that forwards argv", async () => {
		...
		const content = files.get("/Users/me/.config/fish/conf.d/stp-profiles.fish") ?? "";
		expect(content).toContain("function stp-work --wraps stp");
		expect(content).toContain("command stp --profile=work $argv");
	});
	...
	it("installs the fish alias under XDG_CONFIG_HOME when set", async () => {
		...
		expect(result.configPath).toBe("/home/me/.dotfiles/config/fish/conf.d/stp-profiles.fish");
		expect(files.get(result.configPath)).toContain("function stp-work --wraps stp");
	});
	...
	it("writes a PowerShell function because aliases cannot carry arguments", async () => {
		...
		expect(content).toContain("function stp-work");
		expect(content).toContain("& stp --profile=work @args");
	});
	...
	it("detects pwsh from PSModulePath when SHELL is unset on Windows", async () => {
		...
		expect(files.get(result.configPath)).toContain("& stp --profile=work @args");
	});
	...
	it("replaces a previous block for the same alias", async () => {
		const files = new Map<string, string>([
			[
				...
				[
					"before",
					"# >>> stp profile alias: stp-work >>>",
					"alias stp-work='command stp --profile=old'",
					"# <<< stp profile alias: stp-work <<<",
					"after",
				].join("\n"),
			],
		...
		expect(content).toContain('command stp --profile=work "$@"');
	...
	it("refuses to rewrite a malformed managed block missing its end marker", async () => {
		...
		const original = ["# >>> stp profile alias: stp-work >>>", "stp-work() {", "export SECRET=keepme"].join("\n");
	...
	it("refuses to shadow the base stp command case-insensitively", async () => {
		for (const aliasName of ["stp", "STP"]) {
			await expect(
				installProfileAlias({
					profile: "work",
					aliasName,
	...
	it("validates profile names before rendering shell code", async () => {
		...
		).rejects.toThrow("Invalid STP profile");
	...
	it("normalizes backslashes in XDG_CONFIG_HOME for fish config paths on Windows", async () => {
		...
		expect(result.configPath).toBe("D:/xdg/fish/conf.d/stp-profiles.fish");
		expect(result.reloadedWith).toBe("source 'D:/xdg/fish/conf.d/stp-profiles.fish'");
	});
