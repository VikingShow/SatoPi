...
import { RpcSubagentRegistry, readRpcSubagentTranscript } from "@satopi/pi-coding-agent/modes/rpc/rpc-subagents";
import type { RpcSubagentFrame } from "@satopi/pi-coding-agent/modes/rpc/rpc-types";
import {
	type AgentProgress,
...
} from "@satopi/pi-coding-agent/task";
...
describe("readRpcSubagentTranscript", () => {
	test("returns complete JSONL entries and byte cursor", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "stp-rpc-subagent-transcript-"));
		tempPaths.push(dir);
	...
	test("returns empty cursor result for missing transcript files", async () => {
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "stp-rpc-subagent-transcript-missing-"));
		tempPaths.push(dir);
