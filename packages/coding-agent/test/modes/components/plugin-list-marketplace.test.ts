...
import { PluginManager } from "@satopi/pi-coding-agent/extensibility/plugins";
import {
...
} from "@satopi/pi-coding-agent/modes/components/plugin-settings";
...
describe("MarketplacePluginDetailComponent", () => {
	...
	it("shortens home-relative install paths to ~ before rendering", () => {
		...
		const installPath = `${home}/.stp/cache/plugins/sample@mkt`;
		...
		expect(text).toContain("~/.stp/cache/plugins/sample@mkt");
