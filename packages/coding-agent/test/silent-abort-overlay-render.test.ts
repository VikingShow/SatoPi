...
function makeSubagentRegistry(sessions: ObservableSession[]) {
	...
	} as unknown as import("@satopi/pi-coding-agent/modes/session-observer-registry").SessionObserverRegistry;
}
...
describe("Agent hub silent-abort regression", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "stp-overlay-test-"));
	});
