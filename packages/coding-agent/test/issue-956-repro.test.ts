...
import { getConfigRootDir, getProjectDir, removeWithRetries, setAgentDir, setProjectDir } from "@satopi/pi-utils";
...
describe("issue #956: interactive /mcp test", () => {
	...
	beforeEach(async () => {
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "stp-issue-956-project-"));
		agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "stp-issue-956-agent-"));
