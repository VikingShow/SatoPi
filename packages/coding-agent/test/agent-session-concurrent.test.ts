...
import { Agent, AgentBusyError, type AgentMessage, type AgentTool } from "@satopi/pi-agent-core";
import type { AssistantMessage, Message, ToolCall } from "@satopi/pi-ai";
...
describe("AgentSession TTSR resume gate", () => {
	...
	it("relativizes the rule file path in the TTSR interrupt injection (no absolute leak)", async () => {
		...
		const ruleAbsPath = path.join(cwd, ".stp", "rules", "no-unwrap.md");
