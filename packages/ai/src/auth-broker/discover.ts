...
function resolveSnapshotTtlMs(): number {
	const raw = process.env.STP_AUTH_BROKER_SNAPSHOT_TTL_MS;
	...
	logger.warn("Invalid STP_AUTH_BROKER_SNAPSHOT_TTL_MS; using default", { value: raw });
 ...
 *
 * 1. `STP_AUTH_BROKER_URL` / `STP_AUTH_BROKER_TOKEN` env vars.
...
export async function resolveAuthBrokerConfig(
	...
	const envUrl = process.env.STP_AUTH_BROKER_URL;
	const envToken = process.env.STP_AUTH_BROKER_TOKEN;
	...
	if (!token) {
		throw new AIError.MissingApiKeyError(
			undefined,
			`STP_AUTH_BROKER_URL is set (${url}) but no bearer token is available. ` +
				`Set STP_AUTH_BROKER_TOKEN, the \`auth.broker.token\` config entry, or place one at ${getAuthBrokerTokenFilePath()}.`,
		);
	}
	return { url, token };
}
