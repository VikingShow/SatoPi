...
(`~/.stp/agent/sessions/`). SQLite-backed; data is synced once into the same
`~/.stp/stats.db` that `packages/stats` uses, then queried by short Python
scripts.
...
scripts/session-stats/
  sync.py          # walks ~/.stp/agent/sessions/ and populates ss_* tables
...
credentials resolved through stp's auth storage — stored key, OAuth, or env
...
Verdicts are cached in `~/.stp/stats-audit-cache.json` (keyed by session id +
