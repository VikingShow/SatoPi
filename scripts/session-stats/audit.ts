#!/usr/bin/env bun
/**
 * Token-usage audit over the local stp session corpus (~/.stp/agent/sessions/).
 *
 ...
 * asks a small model (default: anthropic/claude-sonnet-4-6 via @satopi/pi-ai)
 * to judge:
 ...
 *
 * Auth: resolves an API key for the classifier provider through stp's auth
 * storage (~/.stp/agent/agent.db: stored key, OAuth, or env var fallback).
 */
...
import { getAgentDbPath, isEnoent } from "@satopi/pi-utils";
import SYSTEM_PROMPT from "./audit-prompt.md" with { type: "text" };

const SESSIONS_ROOT = path.join(os.homedir(), ".stp", "agent", "sessions");
const DEFAULT_MODEL = "anthropic/claude-sonnet-4-6";
const CACHE_PATH = path.join(os.homedir(), ".stp", "stats-audit-cache.json");
...
function parseCli(argv: string[]): CliOptions {
	...
	if (values.help) {
		console.log(
			`session audit — token usage analysis over ~/.stp/agent/sessions\n\n` +
				...
				`  --no-cache               skip the verdict cache (~/.stp/stats-audit-cache.json)\n` +
