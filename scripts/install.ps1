# STP Coding Agent Installer for Windows
# Usage: irm https://raw.githubusercontent.com/VikingShow/SatoPi/main/scripts/install.ps1 | iex
#
# Or with options:
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/VikingShow/SatoPi/main/scripts/install.ps1))) -Source
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/VikingShow/SatoPi/main/scripts/install.ps1))) -Binary
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/VikingShow/SatoPi/main/scripts/install.ps1))) -Source -Ref v3.20.1
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/VikingShow/SatoPi/main/scripts/install.ps1))) -Source -Ref main
#   & ([scriptblock]::Create((irm https://raw.githubusercontent.com/VikingShow/SatoPi/main/scripts/install.ps1))) -Binary -Ref v3.20.1
...
$Repo = "VikingShow/SatoPi"
$Package = "@satopi/pi-coding-agent"
$InstallDir = if ($env:PI_INSTALL_DIR) { $env:PI_INSTALL_DIR } else { "$env:LOCALAPPDATA\stp" }
$BinaryName = "stp-windows-x64.exe"
...
function Configure-BashShell {
    try {
        $settingsDir = Join-Path $env:USERPROFILE ".stp\agent"
        ...
        if ($bashPath) {
            ...
            Write-Host "  STP requires a bash shell on Windows. Options:" -ForegroundColor Yellow
...
function Install-ViaBun {
    ...
    if ($Ref) {
        ...
        $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("stp-install-" + [System.Guid]::NewGuid().ToString("N"))
    ...
    Write-Host ""
    Write-Host "✓ Installed stp via bun" -ForegroundColor Green

    Configure-BashShell

    Write-Host "Run 'stp' to get started!"
}
...
function Install-Binary {
    ...
    $OutPath = Join-Path $InstallDir "stp.exe"
    Invoke-WebRequest -Uri $BinaryUrl -OutFile $OutPath -TimeoutSec 900

    Write-Host ""
    Write-Host "✓ Installed stp to $OutPath" -ForegroundColor Green
    ...
    if ($needsRestart) {
        Write-Host "Restart your terminal, then run 'stp' to get started!"
    } else {
        Write-Host "Run 'stp' to get started!"
    }
}
