...
const SUMS = {
	"stp-darwin-arm64": "darwin_arm64_sha",
	"stp-darwin-x64": "darwin_x64_sha",
	"stp-linux-arm64": "linux_arm64_sha",
	"stp-linux-x64": "linux_x64_sha",
};
...
describe("renderFormula", () => {
	...
	// outside the staging CWD, `Dir["stp-*"].first` returns `nil`, and
	// `bin.install nil => "stp"` raises (issue #2398).
	it("attaches `using: :nounzip` to every per-platform url stanza", () => {
		const matches = formula.match(/using: :nounzip/g) ?? [];
		expect(matches).toHaveLength(4);
		for (const arch of ["stp-darwin-arm64", "stp-darwin-x64", "stp-linux-arm64", "stp-linux-x64"]) {
	...
	// popened binary doesn't touch the real `~/.stp` (denied by Homebrew's
	...
	it("wraps `generate_completions_from_executable` with a HOME redirect to buildpath", () => {
		expect(formula).toMatch(
			/with_env\(HOME: buildpath\) do\n\s+generate_completions_from_executable\(bin\/"stp", "completions", shells: \[:bash, :zsh, :fish\]\)\n\s+end/,
		);
