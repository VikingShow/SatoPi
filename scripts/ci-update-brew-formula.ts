#!/usr/bin/env bun
//
// Render the Homebrew formula for `stp` from a published GitHub release and write
// it to a tap checkout. The release publishes per-platform bare binaries
// (stp-<platform>-<arch>); this reads their sha256 digests straight from the
// release metadata so the formula never drifts from the shipped assets.
//
// Usage:
//   bun scripts/ci-update-brew-formula.ts <tag> --out <path/to/Formula/stp.rb>

import { $ } from "bun";

const REPO = process.env.STP_REPO ?? "VikingShow/SatoPi";
const HOMEPAGE = "https://stp.sh";
const DESC = "Coding agent with the IDE wired in (stp)";
...
export function renderFormula(version: string, sums: Record<string, string>): string {
	...
	// which nests the file outside the staging CWD; `Dir["stp-*"].first` then
	// returns `nil` and `bin.install nil => "stp"` raises.
	//
	...
	// not touch the real `/Users/<user>/.stp` (denied by Homebrew's sandbox
	// profile, which would otherwise fail the popen).
	return `class Stp < Formula
  desc "${DESC}"
  homepage "${HOMEPAGE}"
  version "${version}"
  license "MIT"
  ...
  on_macos do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/stp-darwin-arm64",
          using: :nounzip
      sha256 "${sums["stp-darwin-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/stp-darwin-x64",
          using: :nounzip
      sha256 "${sums["stp-darwin-x64"]}"
    end
  end

  on_linux do
    on_arm do
      url "https://github.com/${REPO}/releases/download/v#{version}/stp-linux-arm64",
          using: :nounzip
      sha256 "${sums["stp-linux-arm64"]}"
    end
    on_intel do
      url "https://github.com/${REPO}/releases/download/v#{version}/stp-linux-x64",
          using: :nounzip
      sha256 "${sums["stp-linux-x64"]}"
    end
  end

  def install
    bin.install Dir["stp-*"].first => "stp"
    (bin/"stp").chmod 0555
    with_env(HOME: buildpath) do
      generate_completions_from_executable(bin/"stp", "completions", shells: [:bash, :zsh, :fish])
    end
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/stp --version")
  end
end
`;
}
...
async function main(): Promise<void> {
	...
	const targets = ["stp-darwin-arm64", "stp-darwin-x64", "stp-linux-arm64", "stp-linux-x64"];
