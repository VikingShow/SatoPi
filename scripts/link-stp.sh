#!/bin/sh
# Install the dev `stp` wrapper into Bun's global bin directory.
#
...
# `packages/coding-agent/scripts/stp`. See that wrapper's header comment for the
...
# through that error would expand `$(bun pm -g bin)/stp` to `/stp` and try to
# write under `/` — see https://github.com/VikingShow/SatoPi/issues/3701.
set -e

repo_root=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd -P)
target=$repo_root/packages/coding-agent/scripts/stp

if [ ! -x "$target" ]; then
	echo "link-stp: target wrapper not found or not executable: $target" >&2
	exit 1
fi
...
mkdir -p "$global_bin"
ln -sfn "$target" "$global_bin/stp"
echo "link-stp: linked $global_bin/stp -> $target"
