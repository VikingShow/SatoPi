...
```ts
import type { HookAPI } from "@satopi/pi-coding-agent/extensibility/hooks";
...
`src/extensibility/hooks/index.ts` and the package subpath `@satopi/pi-coding-agent/extensibility/hooks` export:
...
The package root (`@satopi/pi-coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
