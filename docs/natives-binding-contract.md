...
This document defines the JS/TS contract between `@satopi/pi-natives` callers and the loaded N-API addon.
...
Consumers in `packages/coding-agent` and `packages/tui` import directly from `@satopi/pi-natives`.
