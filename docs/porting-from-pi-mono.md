...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/pi-coding-agent` → `@satopi/pi-coding-agent`
  - `@mariozechner/pi-agent-core` → `@satopi/pi-agent-core`
  - `@mariozechner/pi-tui` → `@satopi/pi-tui`
  - `@mariozechner/pi-ai` → `@satopi/pi-ai`
  - `@mariozechner/pi-utils` → `@satopi/pi-utils`
- Some upstream packages publish under the `@earendil-works/*` scope instead of `@mariozechner/*`. Map it the same way (`@earendil-works/pi-coding-agent` → `@satopi/pi-coding-agent`, and so on).
- The bare `typebox` package is not an `@satopi/*` scope; do not rewrite it as one. See the Extensions divergence in section 15 for how tool-parameter schemas map.
...
- In `packages/coding-agent`, use `logger` from `@satopi/pi-utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@satopi/pi-natives` exports generated native bindings from `packages/natives/native`.
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@satopi/pi-natives` | Native implementation with a small TS wrapper |
...
| `formatSize` from `pi-coding-agent`                              | `formatBytes` from `@satopi/pi-utils`                                                            |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@satopi/pi-natives`
