...
Most runtime lookups use `$env` from `@satopi/pi-utils` (`packages/utils/src/env.ts`).
...
These are consumed via `@satopi/pi-utils/dirs` and affect where coding-agent stores data.
