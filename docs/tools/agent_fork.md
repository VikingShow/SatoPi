...
- Key collaborators:
  - `packages/coding-agent/src/agent/agent-fork-manager.ts` — `AgentForkManager` child-agent creation and subtask distribution
  - `packages/coding-agent/src/config/settings-schema.ts` — `magicKeywords.swarm.maxForkDepth` nesting limit (default 1: children cannot fork further)
...
2. Spawns `count` child agents via `AgentForkManager`, each inheriting the parent's context.
3. Enforces the configured nesting-depth limit (`magicKeywords.swarm.maxForkDepth`).
