...
`@satopi/pi-natives` and the embedded shell/PTY runtime. They are intentionally
internal: end users see `@satopi/pi-natives` exports, not these crate APIs.
...
- Native API surface and loader (`@satopi/pi-natives`):
...
package (`@satopi/pi-natives`) and the architecture pages above. Promote a
