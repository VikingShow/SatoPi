...
This document covers the runtime loader shipped by `@satopi/pi-natives`: how `native/index.js` decides which `.node` file to require, how compiled-binary embedded payloads are extracted, and what startup failures report.
...
For install and compiled-binary paths, the loader verifies a release sentinel export with a fixed name (`__piNativesSatoPi`, decoupled from the package version so a release bump never renames it). Workspace-dev loads skip this validation so a local checkout can rebuild after a pull. The loader does not validate the full export surface; stale same-version or incomplete binaries still surface as missing members or native errors at use sites.
...
- **Core directories**:
  - `leafPackageDir`: directory of the platform leaf package, resolved via `require.resolve("@satopi/pi-natives-<tag>/package.json")`; `null` when no leaf is installed (e.g. local dev) and forced to `null` in compiled-binary mode.
...
- release sentinel mismatch details when a loadable `.node` belongs to another `@satopi/pi-natives` version.

### Non-compiled startup failures

Normal package/runtime diagnostics include:

- reinstall hint (`bun install @satopi/pi-natives`),
