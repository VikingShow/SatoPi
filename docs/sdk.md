...
The SDK is the in-process integration surface for `@satopi/pi-coding-agent`.
...
```bash
bun add @satopi/pi-coding-agent
```

## Entry points

`@satopi/pi-coding-agent` exports the SDK APIs from the package root (and also via `@satopi/pi-coding-agent/sdk`).
...
```ts
import { createAgentSession } from "@satopi/pi-coding-agent";
...
```ts
import { createAgentSession, SessionManager } from "@satopi/pi-coding-agent";
...
```ts
import { SessionManager } from "@satopi/pi-coding-agent";
