...
> **TODO(satopi):** this whole `infra/` tree documents upstream oh-my-pi's
> self-hosted ARC + Kata + k3s runner cluster. This repo is a standalone copy
> (`isFork: false`) with zero self-hosted runners — CI now runs on GitHub-hosted
> runners (see `.github/workflows/ci.yml`). Everything below is dead legacy and a
